...
- The `enforce-test-command` Bash guard no longer reads an argument's contents as commands. It judged the whole command string, so quoted text and here-document bodies were split on their own pipes, parens and line breaks — and a `gh pr create --body "$(cat <<'EOF' … EOF)"` describing a test change got denied for the prose inside it. Quoted strings and here-document bodies are now blanked before the split; command substitutions stay visible, so `echo "$(pest)"` is still caught. The scan walks a character array rather than indexing the string, so a consumer's Pint ruleset rewriting `strlen`/`substr` to their `mb_` forms cannot leave it counting characters while indexing bytes — which would end the scan at the first non-ASCII byte.
