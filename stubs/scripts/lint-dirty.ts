...
try {
  // Extract modified (excluding deleted) and untracked files natively
  const cmd: string =
  ...
  ).filter((f) => existsSync(f)); // Bulletproof safety constraint
  ...
  // Execute concurrently generically via bunx
  ...
  // Ensure the command fails safely if linters find unfixable errors
  process.exit(1);
}
